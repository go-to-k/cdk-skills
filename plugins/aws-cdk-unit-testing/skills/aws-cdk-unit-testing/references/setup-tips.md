...
`cdk.json` には `cdk init` 時点のデフォルトが書き込まれ、その後 **CDK をバージョンアップしても自動更新されない**。そのため手動で何も触っていなくても、ライブラリ内部の現行デフォルトと乖離しうる。
「フラグを手動で追加したか」で判定する意味は薄く、**スナップショットテストを採用しているなら入れておく**のが安全。
